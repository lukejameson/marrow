<script lang="ts">
  ...
  async function handleNewChat() {
    // Prevent any session loading while transitioning
  ...
  async function searchMentions(query: string) {
    ...
    mentionLoading = true;
    try {
      const results = await apiClient.searchRecipesForMention({
        query,
        limit: 5,
      });
  ...
  onMount(async () => {
    ...
    loadingPrompts = true;
    try {
      const result = await apiClient.getPersonalizedSuggestions();
  ...
  $effect(() => {
    ...
    } else if (sessionId && sessionId === currentSessionId && isLoadingSession) {
      // Session was just created and URL updated - clear loading flag
      isLoadingSession = false;
    } else if (!sessionId && !isLoadingSession && (currentSessionId || messages.length > 0)) {
      // URL has no session - reset to new chat state
      messages = [];
      currentSessionId = null;
      error = '';
      pendingImages = [];
  ...
  $effect(() => {
    // Access messages to trigger the effect
    const messageCount = messages.length;
    const lastMessage = messages[messageCount - 1];

    // Scroll to bottom on next tick to ensure DOM is updated
    tick().then(() => {
  ...
  async function loadSession(sessionId: string) {
    ...
    if (isLoadingSession) {
      console.log('⏳ loadSession already in progress, skipping');
      return;
    }
    isLoadingSession = true;
    try {
      const { session, messages: loadedMessages } = await apiClient.getChatSession({ id: sessionId });
      ...
      // Direct assignment - Svelte 5 reactivity handles this efficiently
  ...
  async function sendMessage(content?: string) {
    ...
    if (!currentSessionId && messages.length === 0) {
      isLoadingSession = true; // Prevent effect from triggering loadSession
      try {
        ...
        currentSessionId = session.id;
        await goto(`/generate?session=${session.id}`, { replaceState: true, keepFocus: true });
        // Note: isLoadingSession stays true - URL effect will clear it when session loads
      } catch (err) {
        console.error('Failed to create session:', err);
        isLoadingSession = false; // Only clear on error
      }
    }

    // Create user message object (not added to array yet)
    ...
    // Save user message to database FIRST, then add to local array
    if (currentSessionId) {
      try {
        ...
        const savedUserMessage = await apiClient.addChatMessage({
          sessionId: currentSessionId,
          role: 'user',
    ...
    // Add to local messages array (with ID if saved)
    messages = [...messages, userMessage];

    loading = true;

    try {
      const response = await apiClient.recipeChat({
      ...
      if (currentSessionId) {
        try {
          ...
          const savedAssistantMessage = await apiClient.addChatMessage({
            sessionId: currentSessionId,
            role: 'assistant',
  ...
  async function confirmSaveRecipe() {
    ...
    try {
      ...
      if (!imageUrl && hasImageSearch) {
        ...
        try {
          const imageResult = await apiClient.searchRecipeImages({
  ...
  async function deleteMessage(index: number) {
    ...
    if (messageToDelete.id && currentSessionId) {
      try {
        await apiClient.deleteChatMessage({
...
<style>
  main {
    ...
    overflow: hidden;
    position: relative;
    /* Contain layout to prevent page scroll */
    contain: layout size;
  }

  /* Ensure body doesn't scroll when on this page */
  ...
  .backdrop {
    ...
    z-index: 190;
    animation: fadeIn 0.2s ease-out;
    /* Ensure backdrop captures clicks but allows sidebar to be interactive */
    pointer-events: auto;
  }
  ...
  @media (max-width: 768px) {
    ...
    .container {
      padding: var(--spacing-3);
      padding-top: calc(var(--spacing-3) + 50px); /* Space for toggle button */
      position: relative;
      height: 100%;
      overflow: hidden;
    ...
    .input-section {
      /* Keep input at bottom with safe area */
      flex-shrink: 0;
