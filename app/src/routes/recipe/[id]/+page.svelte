<script lang="ts">
  ...
  async function handleCalculateNutrition() {
    ...
    try {
      ...
      await apiClient.updateRecipe(recipe.id, { nutrition: roundedNutrition });
  ...
  async function handleToggleFavorite() {
    try {
      await apiClient.toggleFavorite($page.params.id);
      loadRecipe();
    } catch (err: any) {
  ...
  async function handleSaveRating() {
    try {
      await apiClient.updateRating($page.params.id, {
  ...
  async function handleMarkAsCooked() {
    try {
      await apiClient.markAsCooked($page.params.id);
      loadRecipe();
    } catch (err: any) {
  ...
  async function handleDelete() {
    ...
    try {
      await apiClient.deleteRecipe($page.params.id);
      goto('/');
    } catch (err: any) {
  ...
  async function handleExplainTechnique(term: string, context?: string) {
    ...
    try {
      const result = await apiClient.explainTechnique({ term, context });
        ...
        {#if recipe.tags && recipe.tags.length > 0}
          <div class="tags">
            {#each recipe.tags as tag}
              <a href={`/collections/tag/${typeof tag === 'string' ? tag : tag.id}`} class="tag">
